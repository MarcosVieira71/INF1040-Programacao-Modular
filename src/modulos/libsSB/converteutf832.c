...
int convUtf8p32(FILE* arquivo_entrada, FILE* arquivo_saida) {
    ...
    int byteInicial;
    while ((byteInicial = fgetc(arquivo_entrada)) != EOF) {
        int qtdBytesCaractere = verificaTamanhoCaractereUtf8(byteInicial);

        if (qtdBytesCaractere == 0) {
            fprintf(stderr, "Caractere UTF-8 inválido no arquivo de entrada.\n");
            return -1;
        }
        ...
        if (!preencheVetorDeBytes(utf8Bytes, qtdBytesCaractere, byteInicial, arquivo_entrada)) {
            fprintf(stderr, "Erro ao preencher vetor de bytes para UTF-8.\n");
            free(utf8Bytes);
            return -1;
        }

        if (!escreveCaractereUtf32(utf8Bytes, qtdBytesCaractere, arquivo_saida)) {
